...
- Breaking: target schema 4 is a lean live-scope/proof-policy record and
  candidate schema 7 is a lean per-invariant evidence record. Old schema-3/6
  records require deliberate review and migration.
- Added optional campaign schema 1 for broad, multi-hypothesis audits instead
  of forcing campaign bookkeeping onto every finding.
- Preserved bounded recovery with `ready`, `recover`, `narrow`, and
  `operator_required` while removing redundant claim-rung state.
- Removed candidate-review sidecars and candidate-level clean-target closure
  machinery from the default flow. One final fresh review now covers the exact
  candidate, report, manifest, and attachments that are ready to hand off.
- Added submission schema 2 as a minimal exact-file manifest. Report semantics
  remain authoritative in `candidate.json` and `report.md` instead of being
  duplicated across title, CVSS, impact, and reproduction metadata.
- Final submission preflight now owns current scope eligibility, current
  severity cap, and accepted proof types within a seven-day freshness window;
  hosted proof vocabulary is mapped rather than compared as raw strings.
- `scripts/validate_hunt.py` is the only validator CLI; the legacy
  `validate-candidate.py` entrypoint was removed.

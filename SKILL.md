...
**Schema 5 — process evidence for ideation, self-review, and exhaustion.** Adds six blocks: `hypothesis_queue` (the ranked campaign ledger — ideas each with a `creativity_signal`, `priority`, and `status`), `intent_corpus` (quoted `intentional_behaviors`/`acknowledged_risks` with a `finding_match`), `adversarial_review` (`advocate` with the eight FP-pattern hits, `cold_verify` verdict + sub-claim decomposition, and the `causal` challenge results), `variant_sweep` (flow shape, siblings and alternate transports checked, variants found), `patch_bypass` (the seven bypass vectors against a known fix), and `exhaustion` (what was `tried`, and the five `depth_contract` records). **Report stage requires `schema_version >= 5`.** A `REPORTABLE` schema-5 candidate must show the passes actually ran, not merely be present: `intent_corpus` with a `checked_at`, listed `sources`, and `finding_match != "intentional"`; an `adversarial_review.advocate` that recorded its `layers_checked` and a `strongest_defense` and does not set `blocks: true`; a `cold_verify.verdict == "CONFIRMED"` carrying a re-derived severity, a null `killed_subclaim`, and a `subclaims[]` decomposition (≥2 links, each `supported` — the explicit re-derivation, not a bare self-verdict); and a written, evidenced `rebuttal` for every `advocate.fp_pattern_hits[]` entry. A schema-5 `NO_REPORTABLE_FINDING` requires the `exhaustion` record — `tried[]` and all five `depth_contract` fields — so "the invariant held" costs the same articulated evidence a report does. Empty or contradictory fields never satisfy a gate; that is the "field satisfiable without the work" failure schema 4 closed for the refutation and novelty blocks.

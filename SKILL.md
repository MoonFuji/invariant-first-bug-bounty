...
**Schema 5 (v0.4.0) — process evidence for ideation and self-review.** Adds five blocks: `hypothesis_queue` (the ranked campaign ledger — ideas each with a `creativity_signal`, `priority`, and `status`), `intent_corpus` (quoted `intentional_behaviors`/`acknowledged_risks` with a `finding_match`), `adversarial_review` (`advocate` with the eight FP-pattern hits, `cold_verify` verdict, and the `causal` challenge results), `variant_sweep` (flow shape, siblings and alternate transports checked, variants found), and `patch_bypass` (the seven bypass vectors against a known fix). **Report stage requires `schema_version >= 5`.** A `REPORTABLE` schema-5 candidate must show both passes actually ran, not just present: `intent_corpus` with a `checked_at`, listed `sources`, and `finding_match != "intentional"`; an `adversarial_review.advocate` that recorded its `layers_checked` and a `strongest_defense` and does not itself set `blocks: true`; a `cold_verify.verdict == "CONFIRMED"` carrying a re-derived severity and a null `killed_subclaim`; and a written, evidenced `rebuttal` for every `advocate.fp_pattern_hits[]` entry. Empty or contradictory fields do not satisfy the gate — that is the "field satisfiable without the work" failure schema 4 closed for the refutation and novelty blocks.

Legacy schema-3/4 candidates still validate at the model and decision stages, but report stage requires schema 5 — migrate a candidate before a REPORTABLE report so the ideation and self-review gates apply.

...
7. **Independent review — do not self-certify.** The review (`references/adversarial-self-review.md`: Advocate across five protection layers + eight FP patterns, Cold-verifier re-deriving severity from MEDIUM, Causal-challenger's intervention/counterfactual/confounder) must be run by an **independent, fresh-context agent** — spawn one, hand it only the candidate and the repo, with none of your prosecution narrative — and record its identifier in `candidate.json.adversarial_review.reviewer`. Self-review over-rates its own findings (measured). If your harness cannot spawn an independent agent, set `reviewer: "owed"`, **record that an independent review is owed**, and treat the verdict as provisional. A self-graded review does **not** certify a final verdict: the validator rejects `reviewer: "self"` on `REPORTABLE` and `NO_REPORTABLE_FINDING`. You may still run the rotation yourself first to strengthen the candidate, but it is preparation, not certification.

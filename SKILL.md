...
Before recon, persist the commitment in `candidate.json.commit`: the operating `mode`, the one `invariant` you will test, the `expected_delta` an attacker should gain, and `committed_at`. State the terminal verdict and `decision.gate` out loud at the end. The snapshot is immutable — it is what a later drift is measured against. If `model.security_invariant` ends up different from `commit.invariant`, that reframe must be recorded (a `decision_history` entry or `commit.superseded_by`) or the validator rejects it: a quiet switch to a different bug than the one you committed to is caught, not just an abandoned invariant or a silent slide from `HOLD` toward `REPORTABLE`. You do not get to change a declared verdict without appending to `decision_history` and saying why.
...
**Schema 5 — process evidence for the commitment, ideation, self-review, and exhaustion.** Adds seven blocks: `commit` (the immutable pre-hunt snapshot — `mode`, `invariant`, `expected_delta`, `committed_at`), `hypothesis_queue` (the ranked campaign ledger — ideas each with a `creativity_signal`, `priority`, and `status`), `intent_corpus` (quoted `intentional_behaviors`/`acknowledged_risks` with a `finding_match`), `adversarial_review` (`advocate` with the eight FP-pattern hits, `cold_verify` verdict + sub-claim decomposition, and the `causal` challenge results), `variant_sweep` (flow shape, siblings and alternate transports checked, variants found), `patch_bypass` (the seven bypass vectors against a known fix), and `exhaustion` (what was `tried`, and the five `depth_contract` records). **Report stage requires `schema_version >= 5`.** Every schema-5 candidate must carry `commit`, and a committed invariant that differs from `model.security_invariant` without a recorded reframe is rejected. A `REPORTABLE` schema-5 candidate must additionally show the passes actually ran, not merely be present: `intent_corpus` with a `checked_at`, listed `sources`, and `finding_match != "intentional"`; an `adversarial_review.advocate` that recorded its `layers_checked` and a `strongest_defense` and does not set `blocks: true`; a `cold_verify.verdict == "CONFIRMED"` carrying a re-derived severity, a null `killed_subclaim`, and a `subclaims[]` decomposition (≥2 links, each `supported` — the explicit re-derivation, not a bare self-verdict); a written, evidenced `rebuttal` for every `advocate.fp_pattern_hits[]` entry; and, when every public novelty check comes back clean, a `private_duplicate_risk` of `medium` or `high`, never `low` (absence of public matches is not evidence about the invisible private pool). It must also carry a `target.saturation` assessment, and a non-disclosing program (`discloses_reports: false` — zero public dedup signal) forces `private_duplicate_risk: high`, because duplicates, the dominant failure mode, are decided at target selection. A schema-5 `NO_REPORTABLE_FINDING` requires the `exhaustion` record — `tried[]` and all five `depth_contract` fields — plus a present `intent_corpus` (built in step 1, not back-filled), so "the invariant held" costs the same articulated evidence a report does. Empty or contradictory fields never satisfy a gate; that is the "field satisfiable without the work" failure schema 4 closed for the refutation and novelty blocks.

...
Persist the commitment in `candidate.json.commit` — two parts, two moments. `mode` and `committed_at` are fixed **before recon**; the one `invariant` you will test is recorded **when you promote it** (workflow step 3), so an early terminal that dies at scope owes no invariant yet. State the terminal verdict and `decision.gate` out loud at the end. The snapshot is immutable and drift is measured against it: a `commit.mode` that ends up different from `target.operating_mode` (a silent mode switch) and a `commit.invariant` different from `model.security_invariant` with no recorded `commit.superseded_by` (a silent reframe — a quiet switch to a different bug than the one you committed to) are both rejected. A `decision_history` verdict change is not an invariant-reframe record. You do not get to change a declared verdict without appending to `decision_history` and saying why.

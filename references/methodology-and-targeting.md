...
**Saturation is a strong prior, but the finding's *collision rate* is what actually duplicates — not bug obviousness.** In the mined failure history every adjudicated duplicate sat on a marquee/high-volume or *non-disclosing* program, and even deliberately non-obvious findings duped there; obviousness only amplifies. So (1) prefer a disclosing, low-volume program where a public dedup is even possible, and treat a non-disclosing program as `private_duplicate_risk` ≥ `medium` by default (you cannot see the private pool). But (2) — the load-bearing nuance when **every program is swarmed** — a bespoke finding with a genuinely low collision rate still pays on a saturated marquee program: the two wins in the history paid on a program at r90≈647. That low-collision profile is the Tier-3 cross-layer / no-advisory vein (`emerging-surfaces` §2B; `hypothesis-generation`, the cross-layer-oracle archetype). So when you cannot out-*select* saturation, out-*create* it: an obvious-class finding on a swarmed program rotates; a Tier-3 finding on the same program is the play. Novelty is the tie-breaker within a well-chosen target — and the whole game on a universally-swarmed one.

Two honest limits on this note. It is **one hunter's HackerOne distribution (n=37), not a universal law** — re-mine before importing the prior to a different platform or program type (Bugcrowd, disclosing-only, source-only). And on a non-disclosing program, `novelty.classification: distinct` means *distinct from the **public** pool only* — the private pool is invisible, so `distinct` there is not evidence of novelty. What carries a high-dup-context finding to REPORTABLE is not `distinct`; it is the `novelty.collision_differentiator` — the articulated reason this finding is low-collision despite the swarm (validator: high risk / non-disclosing / hot cluster requires it).
